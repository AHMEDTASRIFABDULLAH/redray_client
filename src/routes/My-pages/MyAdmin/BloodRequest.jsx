...
const BloodRequest = () => {
  ...
  const { data, refetch } = useQuery({
    queryKey: ["alldonations", sort],
    queryFn: async () => {
      const { data } = await axiosPublic.get(
        `/all-blood-donations?sort=${sort}`
      );
      return data;
    },
  });
  ...
  return (
    <div className="p-4">
      <h1 className="text-center text-2xl flex justify-center items-center gap-3 font-bold p-4 bg-gradient-to-r from-red-100 to-red-200 shadow-md rounded-md">
        <FaClover className="text-green-600 text-3xl" /> All Donation Requests
      </h1>
      <div className=" my-6 ">
        <select
          name="sort"
                  ...
                  {isAdmin === "admin" ? (
                    <td className="px-3 py-2 flex flex-col justify-center items-center gap-2">
                      <Link to={`/dashboard/edit/${d._id}`}>
                        <button className="w-24 btn btn-xs flex items-center justify-center gap-1 px-3 py-1 bg-blue-500 text-white rounded shadow hover:bg-blue-600">
                          <FaEdit /> Edit
                        </button>
                      </Link>
                      <button
